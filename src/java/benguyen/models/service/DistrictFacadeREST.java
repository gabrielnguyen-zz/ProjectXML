...
public class DistrictFacadeREST extends AbstractFacade<District> {
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public void edit(@PathParam("id") Integer id, District entity) {
        super.edit(entity);
    }
    ...
    public District find(@PathParam("id") Integer id) {
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<District> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
